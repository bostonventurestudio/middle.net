...
export function copyLinkToClipboard() {
    if (this.state.slug === "") {
        ...
        if (copy(`${window.location.origin}/solution-1/${this.state.slug}`)) {
        ...
        if (results.length < 5) {
            if (this.state.searchRadius === this.state.maxRadius) {
                ...
                this.setState({searchRadius: radius}, () => {
                    this.sendNearbyPlacesAPIRequest(this.state.searchRadius, this.state.center, this.setNearbyPlaces);
                });
            }
        } else {
            this.setState({totalNearbyPlaces: results});
...
export function setCenterAndNearbyPlaces() {
    ...
    this.setState({
        ...
        this.sendNearbyPlacesAPIRequest(this.state.searchRadius, this.state.center, this.setNearbyPlaces);
        this.sendNearbyPlacesAPIRequest(maxRadius, this.state.center, this.setHeatMapData);
    });
}

export function setNearbyPlacesAroundPseudoCenter(center) {
    this.setState({
        totalNearbyPlaces: [],
        nearbyPlacesIndex: 0,
        nearbyPlaces: new Array(5),
        ...
        this.sendNearbyPlacesAPIRequest(this.state.searchRadius, center, this.setNearbyPlaces);
    });
}

export function sendNearbyPlacesAPIRequest(radius, center, callback) {
